...
my $base;
while (@filenames) {
	my $gbffile=shift @filenames;
	my $target_name=substr($gbffile,0,rindex($gbffile,"\."));
	$target_name=~ s/\s/_/g;
	my $filename=substr($target_name,rindex($target_name,"\/")+1);
	#printf("(%d)\tNow processing file => %s\n",++$i,$gbffile);
	$base = $output_directory."/".substr($gbffile,rindex($gbffile,"/")+1);
	open (my $GBF_IN,"<",$gbffile) or die "Can't open $gbffile: $!";
	...
	open ($FSA_OUT,">","$base.fsa") or die "Unable to open sequence output file\n";
	open ($TBL_OUT,">","$base.tbl") or die "Unable to open feature table output file\n";
	open ($ERR_OUT,">","$base.err") or die "Unable to open error output file\n";
	...
	while ($thisline = <$GBF_IN>) {
		...
		if ($thisline =~ /^LOCUS\s+(\S*).*$/ || $thisline =~ /^ID\s+(\S*);.*$/ || $thisline =~ /^ID\s+(\S*)\s+SV\s+\d+;.*$/) {
			# record locus
			$locus = $filename;
...
my $item2="locus_tag";
my $item3="protein_id";
my $item4="transcript_id";
my $item5="anticodon";
my $item6="trans_splicing";

my $infile;
my $outfile;

while (defined ($infile=glob ($output_directory."/*.tbl"))) {
	tie my @array,"Tie::File",$infile;
	@array=grep !/$item1|$item2|$item3|$item4|$item5|$item6/, @array;
	untie @array;

	#$outfile = $output_directory."/"."new_".substr ($infile,rindex ($infile,"/")+1);
	#$outfile=~ s/new_new_//g;
	#foreach (<$input>){
	#	next if ($_ =~ /^\t\t\tdb_xref|^\t\t\tlocus_tag|^\t\t\tprotein_id|^\t\t\ttranscript_id|^\t\t\tanticodon|^\t\t\ttrans_splicing/);
	#	print $output $_ ;
	#}
	#close $input;
	#close $output;
...
=head1 NAME
    ...
    gbf2tbl_v1.pl version 1.0

=head1 DESCRIPTION

    generate .tbl and .fsa files from .gb/.gbf/.gbk file(s)
...
=head1 SYNOPSIS
    ...
    [-i -input]        required: (default: input) input directory containing .gb file(s).
    [-o -output]       required: (default: output) output directory containing .tbl and .fsa files.
