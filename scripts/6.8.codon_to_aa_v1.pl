...
while (@filenames) {
	...
	open(my $error,">>","$output_directory/bad_codons.txt");
	...
	while (defined ($header=<$input>) and defined ($sequence=<$input>)){
		...
		#for (my $i=0;$i<($length-3);$i+=3){# delete stop codon
			...
			if (exists $hash_codon{$codon}){
				if (($hash_codon{$codon} eq "*") and ($i != ($length-3))) {
					$aa.=$hash_codon{$codon};
					my $j=$i+1;
					print $error "Bad codon $codon in position $j of $filename in species $header!\n";
				}else {
					$aa.=$hash_codon{$codon};
				}
			}else{
		...
		if (substr($aa,-1) eq "*") {
			print $output substr($aa,0,-1)."\n";
		}elsif (substr($aa,-1) ne "*") {
			print $output $aa."\n";
		}
	}
	close $input;
